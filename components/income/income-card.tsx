...
export function IncomeCard({ deck, lens }: { deck: IncomeDeckView; lens: Lens }) {
  ...
  return (
    <div
      className={cn(
        "flex flex-col gap-2.5 rounded-lg border bg-card p-3 ring-1 ring-transparent transition-colors",
        style.ring,
      )}
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <a
            ...
            className="group inline-flex max-w-full items-center gap-1 rounded text-sm font-semibold leading-tight text-foreground hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
            title={`${deck.name} — source`}
          >
            <span className="truncate">{deck.name}</span>
            <ExternalLink className="h-3 w-3 shrink-0 opacity-40 group-hover:opacity-70" aria-hidden />
          </a>
      ...
      <div className="flex items-end justify-between gap-2">
        <div className="font-mono text-lg font-bold leading-none tabular-nums text-foreground">
          ...
          <span className="ml-1 text-[10px] font-medium text-muted-foreground">median</span>
        </div>
        {deck.growthPct > 0 ? (
          <div className="flex items-center gap-0.5 font-mono text-xs font-semibold tabular-nums text-emerald-400">
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-[10px] font-medium">
        <span className="inline-flex items-center gap-1 rounded bg-secondary px-1.5 py-0.5 text-muted-foreground" title="Time to first income">
          <Clock className="h-3 w-3" aria-hidden />
          {timeLabel(deck.timeToFirstIncomeYears)}
        </span>
        <span className="inline-flex items-center rounded bg-secondary px-1.5 py-0.5 text-muted-foreground" title={cap.full}>
          {cap.short}
        </span>
        ...
        <div className="mt-0.5 border-t pt-2">
          ...
          <div className="flex flex-wrap gap-1">
            {deck.exemplars.slice(0, 4).map((ex) => (
              ...
              >
                {ex.name}
                <span className="font-mono text-[10px] text-emerald-400">{ex.rev}</span>
              </a>
            ))}
          </div>
        </div>
